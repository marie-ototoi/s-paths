...
chai.use(sinonChai)
import svgScale from '../../src/svg/scale'
...
describe('svg/scale', () => {
    it('should return the full dimensions of the stage based on the viewbox definition in percentage and the screen dimensions', () => {
        let stage1 = svgScale.scaleStage(displayDef.dev, screens[0])
        let stage2 = svgScale.scaleStage(displayDef.full, screens[0])
        let stage3 = svgScale.scaleStage(displayDef.main, screens[0])
    ...
    it('should return the actual dimensions of the viewbox based on the viewbox definition in percentage and the stage dimensions', () => {
        ...
        let vb1 = svgScale.scaleViewBox(displayDef.dev, stages[0])
        let vb2 = svgScale.scaleViewBox(displayDef.full, stages[0])
        let vb3 = svgScale.scaleViewBox(displayDef.main, stages[1])
        let vb4 = svgScale.scaleViewBox(displayDef.aside, stages[2])
    ...
    it('should return actual values for lists of xPoints and yPoints defined in percentage', () => {
        let grid = svgScale.getGrid(gridDef, stages[1])
